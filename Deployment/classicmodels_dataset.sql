...
-- Add new email attribute in customers table
ALTER TABLE classicmodels.customers
ADD email varchar(100);

-- Assign a unique email for each rows based on customername
DO $$ 
DECLARE
    customernames TEXT[];
    new_email       TEXT;
BEGIN 
    SELECT ARRAY_AGG(c.customername::TEXT) INTO customernames 
    FROM classicmodels.customers AS c;
    FOR i IN ARRAY_LOWER(customernames, 1) .. ARRAY_UPPER(customernames, 1) LOOP
	new_email := REGEXP_REPLACE(customernames[i], '[^A-Za-z\d\s]','','g');
	new_email := REGEXP_REPLACE(new_email, '\s','_','g');
	UPDATE classicmodels.customers
	SET email = CONCAT(LOWER(new_email), '@gmail.com')
	WHERE customername = customernames[i];
    END LOOP;
END $$;

-- Make sure every email address is not null
ALTER TABLE classicmodels.customers
...
-- Assuming checknumber also represents as referenceNumber for online payment

-- Add column called paymentmethod and put "check" as default value
ALTER TABLE classicmodels.payments
ADD paymentmethod character varying(14) NOT NULL DEFAULT 'check';

-- Add constraint to only allow certain types for the payment method
ALTER TABLE ONLY classicmodels.payments
ADD CONSTRAINT payments_paymentmethod_check
CHECK(paymentmethod IN ('check', 'cash', 'debit card', 'credit card', 'online banking'));

----------------------------------------------------------------------------------------------
-- Add auto increment of primary keys for customers
----------------------------------------------------------------------------------------------
DO $$ 
DECLARE
    highest_customernumber INTEGER;
BEGIN 
    SELECT MAX(customernumber)+1 INTO highest_customernumber
    FROM classicmodels.customers;

    CREATE SEQUENCE classicmodels.customernumber_seq
    	INCREMENT 1
    	START 1
    	NO MINVALUE
    	NO MAXVALUE
    ...
    ALTER SEQUENCE classicmodels.customernumber_seq
    OWNER TO postgres;

    ALTER SEQUENCE classicmodels.customernumber_seq
    OWNED BY classicmodels.customers.customernumber;

    ALTER TABLE ONLY classicmodels.customers 
    ALTER COLUMN customernumber SET DEFAULT nextval 
    ('classicmodels.customernumber_seq'::regclass);

    PERFORM pg_catalog.setval('classicmodels.customernumber_seq', highest_customernumber, true);
END $$;
----------------------------------------------------------------------------------------------
-- Add auto increment of primary keys for orders
----------------------------------------------------------------------------------------------
DO $$ 
DECLARE
    highest_ordernumber INTEGER;
BEGIN 
    SELECT MAX(ordernumber)+1 INTO highest_ordernumber
    FROM classicmodels.orders;

    CREATE SEQUENCE classicmodels.ordernumber_seq
        INCREMENT 1
        START 1
        NO MINVALUE
        NO MAXVALUE
    ...
    ALTER SEQUENCE classicmodels.ordernumber_seq
    OWNER TO postgres;

    ALTER SEQUENCE classicmodels.ordernumber_seq
    OWNED BY classicmodels.orders.ordernumber;

    ALTER TABLE ONLY classicmodels.orders 
    ALTER COLUMN ordernumber SET DEFAULT nextval 
    ('classicmodels.ordernumber_seq'::regclass);

    PERFORM pg_catalog.setval('classicmodels.ordernumber_seq', highest_ordernumber, true);
END $$;
